...
plot(surface,axes=FALSE,box=FALSE,col=colo,legend.shrink=0.9,legend.width=2)
points(xy,pch=16,col=gray(0,0.01),cex=1)
mtext(side=3,line=-1,outer=FALSE,cex=0.6,text=paste("Simulated intensity",nrow(xy),"obs"))
...
model<-RMexp(var=2.00000002,scale=25)+RMnugget(var=0)+RMtrend(mean=-1)
...
rsims<-rsims
#par(mfrow=c(1,3))
plot(modsims,axes=FALSE,box=FALSE,zlim=c(0,1),legend.shrink=0.9,legend.width=2,col=colorRampPalette(c("white","black"))(200))
mtext(side=3,line=-1,outer=FALSE,cex=0.6,text="Proportional effort gradient")
eff<-exp(rsims)
#plot(eff)
...
eff2<-eff
eff2[eff2==0]<-NA
plot(eff2,axes=FALSE,box=FALSE,legend.shrink=0.9,legend.width=2,col=colorRampPalette(c("grey90","tomato","darkred","black"))(200))
mtext(side=3,line=-1,outer=FALSE,cex=0.6,text="Realized effort")
#par(mfrow=c(1,1))
...
plot(eff[[1]],col="white",box=FALSE,axes=FALSE,legend=TRUE,legend.shrink=0.9,legend.width=2)
plot(occ_thinsp,pch=16,col=gray(0,0.15),add=TRUE)
mtext(side=3,line=-1,outer=FALSE,cex=0.6,text=paste("Thinned occurences",nrow(occ_thin),"obs"))
...
plot(mapMean, col = colo, axes = FALSE, box = FALSE,xlim=xlim,ylim=ylim,legend.shrink=0.9,legend.width=2)
mtext(side=3,line=-1,outer=FALSE,cex=0.6,text="Predicted surface")
#points(occ_thinsp,pch=1,col=gray(0,0.25),cex=0.6)
#grid(10,6,col=gray(0,0.25),lty=3)
