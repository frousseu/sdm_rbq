...
r<-raster(extent(occ),ncol=100,nrow=100,crs=st_crs(p)$input)
...
region<-as(extent(occ_thinsp),"SpatialPolygons")
...
bpriors<-list(prec=list(default=1/(10)^2,latitude=1/(10)^2,latitude2=1/(10)^2,Intercept=1/(100)^2,sbias=1/(100)^2,effort=1/(100)^2),mean=list(default=0,Intercept=0,latitude=0,latitude2=0,sbias=0,effort=0))

Mesh<-inla.mesh.2d(loc.domain = coordinates(occ_thinsp),max.edge = c(edge,edge*1.5),min.angle = 21,cutoff = edge/2,offset = c(edge,edge*2),crs = crs(region))
...
m2<-ppSpace2(y ~ xx+yy+effort, sPoints = occ_thinsp,
             explanaMesh = explana,
             ppWeight = weight,
...
plot(eff[[1]],xlim=xlim,ylim=ylim,main="Effort (en entier, beaucoup de 0)")
grid(10,6,col=gray(0,0.25),lty=3)
...
temp<-data.frame(eff=values(eff[[1]]),cou=values(rr)[,1],xx=coordinates(rr)[,1],yy=coordinates(rr)[,2])
temp2<-temp[temp$eff>0,]
fit<-glm(cou~xx+yy+eff,data=temp2,family="poisson")
summary(fit)
newdat<-temp
