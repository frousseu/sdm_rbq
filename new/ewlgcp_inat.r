...
bb<-st_bbox(regionll)
dd<-d[decimalLongitude>=bb[1] & decimalLongitude<=bb[3] & decimalLatitude>=bb[2] & decimalLatitude<=bb[4],]

background<-st_as_sf(as.data.frame(dd),coords=c("decimalLongitude","decimalLatitude"),crs=4326)
background<-st_transform(background,st_crs(region))
o<-st_intersects(background,region)
background<-background[lengths(o)>0,]


### filter data
checkpoint("Make filter grid")
g<-rast(extent=ext(region)+5,resolution=5)
g<-setValues(g,1:ncell(g))
ex<-terra::extract(g,vect(background))
background$cell<-ex[,2]
background<-background[!is.na(background$cell),]
dups<-duplicated(st_drop_geometry(background)[,c("species","cell","recordedBy")])
background<-background[!dups,]

obs<-background[background$species == "Cardinalis cardinalis",] 

#plot(st_geometry(dm))
#plot(st_geometry(background),add=TRUE)
#plot(st_geometry(obs),col="forestgreen",add=TRUE)
...
if(FALSE){
  vars<-grep("2",vs,value=TRUE,invert=TRUE)
  me <- MaxEnt(predictorsreg[[vars]], vect(obs),vect(background),removeDuplicates=FALSE,silent=FALSE,args=c("-q","-l","-p"))  
  mer <- mask(predict(me, predictorsreg[[vars]],args=c("outputformat=raw")),vect(na))
...
params<-dmesh_predictors(params,wrap(predictorsreg))
...
#params$effort$nbackgroundadjusted<-ifelse(params$effort$nbackgroundadjusted==20,20000,params$effort$nbackgroundadjusted)
...
vs<-c("tmax","tmax2","conifers_esa","deciduous_esa","crop_esa","mixed_esa","truggedness","grass_esa","builtup_esa")
#vs<-c("tmax","tmax2")
#vs<-paste0(sort(rep(grep("2",vs,value=TRUE,invert=TRUE),2)),c("","2"))

library(car)
dat<-params$predictors
dat$y<-rnorm(nrow(dat))
mo<-lm(formula(paste("y~",paste(vs,collapse="+"))),data=dat)
vif(mo)


#params$effort$nobs[params$effort$nobs>0]<-1
#params$effort[params$effort$nobs>0,]
...
#plot(mask(sdm$spacemean,region),plg=list(size=c(0.8,1)))
#plot(mask(exp(sdm$linkmean-sdm$spacemean),region),plg=list(size=c(0.8,1)))
...
head(params$effort[params$effort$nbackground>0,],100)
