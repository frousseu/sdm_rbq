...
ebirdpath<-list.files("/data/predictors_sdm/expert_maps/eBird/abundance",full=TRUE)
...
lsdms<-list.files("/data/sdm_rbq/rasters",pattern="_birds.tif",full=TRUE)#[-1]
lsdms<-lsdms[rev(order(file.info(lsdms)$mtime))]
sdms<-lapply(lsdms,rast,lyrs="mean")
#sdms<-lapply(sdms,function(i){resample(i,sdms[[1]])})
...
#setdiff(names(sdms),gsub("_ebird.tif","",basename(ebirdpath))

ebird<-rast(lapply(ebirdpath,rast))
names(ebird)<-gsub("_ebird.tif","",basename(ebirdpath))
...
cors<-sapply(seq_len(nlyr(sdms)),function(i){
  print(i)
  cor(values(sdms[[i]])[,1],values(ebird[[i]])[,1],use="complete.obs",method="spearman")
})
cors
hist(cors,xlim=0:1,breaks=20)
...
colnames(m)<-rownames(distm)
rownames(m)<-rownames(distm)



ord<-cmdscale(m,eig=TRUE,k=60)
...
ims<-do.call("c",lapply(seq_along(images),function(x){
  ...
  im<-image_border(im,"white","25x125")
  im<-image_annotate(im,titles[x],size=50,gravity="northwest",location="+40+30",degrees=0,weight=900)
  im<-image_trim(im)
  im<-image_border(im,"white","15x15")
  im<-image_border(im,"grey90","5x5")
  #h<-image_info(im)$heigth
  #w<-image_info(im)$width
  im<-image_fill(im, "antiquewhite", point = "+18+18", fuzz = 0)
  im<-image_border(im,"white","25x25")
  im
}))
